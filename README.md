...
| 4624 |	"An account was successfully logged on" |	Detecting Lateral Movement |	Look for Logon type: ".Logon Type:[\W](3 or 10).*"	Identify successful logon to establish timeline of the remote connection. Logon type 3 is Network (i.e. connection to shared folder on this computer from elsewhere on network. Logon type 10: RemoteInteractive (Terminal Services, Remote Desktop or Remote Assistance). |
| 4625 |	"An account failed to log on" |	Detecting Lateral Movement |	Look for Logon type: ".Logon Type:[\W](3 or 10).*"	Identify unsuccessful logon to establish timeline of the remote connection. Logon type 3 is Network (i.e. connection to shared folder on this computer from elsewhere on network. Logon type 10: RemoteInteractive (Terminal Services, Remote Desktop or Remote Assistance). |
| 4720 |	"A user account was created" |	Malicious account created by attacker |	Identify possible malicious account created by malware or attacker. |	Look at the Security ID to understand what type of account was created:  For local account only: 
- Administrators: S-1-5-32-544
•	Users: S-1-5-32-545
•	Guests: S-1-5-32-546
For domain account only:
•	
o	Admin: S-1-5-21domain-500
o	Domain users: S-1-5-21domain-513
o	Domain admins: S-1-5-21domain-512
o	Enterprise Admins: S-1-5-21root domain-519

Full list of SID available here
Search for activities performed by the created account.
4722	"A user account was enabled"	Account enable by attacker	Identify possible account enable created by malware or attacker.	Look for Look at the Security ID to understand what type of account was enable:
•	Security ID:  The SID of the account. Full list of SID available here
•	Account Name: The account logon name.
•	Account Domain: The domain or - in the case of local accounts - computer name.

Search for activities performed by the enabled account. |
...
| 4657 |	"A registry value was modified.*" |	Persistence mechanism in Windows registry |	Can be used to modify registry key related to persistence mechanism. |	Identify the process doing the modification using the "Process ID" and matching it with event ID 4688.

Look for modification related to the registry keys listed in persistence mechanism:
...
•	HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows NT\CurrentVersion\IniFileMapping\system.inin\boot (WIndows 8.1)
